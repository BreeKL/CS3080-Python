...
def init_weights(X, Y, mode):
    ...
    if mode == 'logistic' or mode == 'linear':
        ...
        weights = np.zeros((n_features, n_classes))  # weights for each class
        bias = np.zeros(n_classes)  # bias for each class
...
def forward_pass(X, weights, bias, mode):
    if mode == 'linear':
        ...
        y1 = sigmoid(y1)  # Activation for hidden layer
        y2 = predict(y1, weights[1], bias[1])
        return [softmax(y2), y1]  # Output layer activation
    else:
        raise ValueError("Unsupported mode")
...
def backward_pass(X, Y, y_pred, weights, bias, lr, mode):
    ...
    if mode == 'logistic' or mode == 'linear':
        ...
        dL2 = y_pred[0] - Y # derivative of XE loss * derivative of softmax simplified
...
def show_loss(Y, y_pred, ep, mode):
    # Optionally print loss
        if mode == 'linear':
            ...
            #y_pred[0] = np.clip(y_pred[0], 1e-15, 1.0) # Avoid log(0)
            loss = -np.mean(Y * np.log(y_pred[0] + 1e-15))
            print(f"Epoch {ep}, Loss: {loss:.4f}")


def train(X, Y, mode='linear'):
    # Training parameters
    lr = 0.1
    epochs = 400
...
def prepare_data(target='Energy Consumption', mode='linear'):

    train_data = pd.read_csv('Assignment4/train_energy_data.csv')
    test_data = pd.read_csv('Assignment4/test_energy_data.csv')
    train_df = pd.DataFrame(train_data)
    test_df = pd.DataFrame(test_data)

    #using pandas to add columns for categorical variables
    ...
    if mode == 'linear':
        ...
        Y_train = Y_train_df.to_numpy(dtype='int')  # Ensure Y is int for classification
...
def save_results(train_df, test_df, y_pred_train, y_pred_test, target, prefix):
    ...
    if isinstance(target, list):
        for i, item in enumerate(target):
            ...
            train_accuracy = np.mean((train_df['Predicted ' + item] == train_df[item]).astype(int))
            print(f"Training Accuracy for {item}: {train_accuracy*100}%")
            test_accuracy = np.mean((test_df['Predicted ' + item] == test_df[item]).astype(int))
            print(f"Test Accuracy for {item}: {test_accuracy*100}%")
    else:
        train_df['Predicted ' + target] = y_pred_train
        test_df['Predicted ' + target] = y_pred_test
        train_accuracy = np.mean((train_df['Predicted ' + target] == train_df[target]).astype(int))
        if train_accuracy > 0: print(f"Training Accuracy for {target}: {train_accuracy*100}%")
        test_accuracy = np.mean((test_df['Predicted ' + target] == test_df[target]).astype(int))
        if test_accuracy > 0: print(f"Test Accuracy for {target}: {test_accuracy*100}% ")
